...
SPARK_DRIVER_MEMORY="16g"               # Driver memory
SPARK_EXECUTOR_MEMORY="80g"             # Executor memory
SPARK_SHUFFLE_PARTITIONS="32"           # Shuffle partitions
...
mkdir -p ${LOG_DIR}
mkdir -p ${OUTPUT_DIR}
mkdir -p ${LOG_DIR}/spark-events/${SLURM_JOB_ID}

# Create Dask temporary directory
export LOCAL_DIR="tmp/$SLURM_JOB_ID"
mkdir -p ${LOCAL_DIR}

export DASK_DIR="${LOCAL_DIR}/dask"
mkdir -p ${DASK_DIR}
...
spark-submit \
    ...
    --conf spark.eventLog.dir=${LOG_DIR}/spark-events/${SLURM_JOB_ID} \
